...
const EditTransactionModal = ({ isOpen, onClose, transaction, onSave }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose}>
            <h3 className="text-xl font-bold text-gray-800 mb-4">Editar Transação</h3>
            <div className="space-y-4">
                <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Valor" className="p-2 border rounded-md w-full" />
                <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Descrição" className="p-2 border rounded-md w-full" />
                <input type="text" value={tag} onChange={e => setTag(e.target.value)} placeholder="Tag" className="p-2 border rounded-md w-full" />
                <input type="date" value={date} onChange={e => setDate(e.target.value)} className="p-2 border rounded-md w-full" />
            </div>
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
            <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg text-center">
                <h1 className="text-3xl font-bold text-gray-800 mb-2">Bem-vindo ao OrçaCasal!</h1>
                <p className="text-gray-600 mb-8">Gerencie as finanças do casal de forma simples e colaborativa.</p>
                <button
                    onClick={signInWithGoogle}
                    className="w-full flex items-center justify-center bg-white border border-gray-300 text-gray-700 font-semibold py-3 px-4 rounded-lg shadow-sm hover:bg-gray-100 transition duration-300"
                >
                    <GoogleIcon />
                    Entrar com Google
                </button>
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
            <div className="max-w-lg w-full bg-white p-8 rounded-xl shadow-lg text-center">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">Configure seu Lar</h2>
                <p className="text-gray-600 mb-6">Crie um novo lar para compartilhar com seu parceiro(a) ou junte-se a um existente usando um ID.</p>
                <div className="space-y-4">
                    ...
                    <div className="relative flex items-center justify-center my-4"><div className="flex-grow border-t border-gray-300"></div><span className="flex-shrink mx-4 text-gray-500">OU</span><div className="flex-grow border-t border-gray-300"></div></div>
                    <div className="flex flex-col sm:flex-row gap-2">
                        <input type="text" value={joinId} onChange={(e) => setJoinId(e.target.value)} placeholder="Insira o ID do Lar" className="flex-grow p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
...
const TransactionForm = ({ householdId, type }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="p-4 bg-gray-100 rounded-lg space-y-3">
            <h3 className="font-bold text-lg">{type === 'weekly' ? 'Adicionar Gasto Semanal' : 'Adicionar Despesa Fixa/Parcelada'}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <input type="text" value={description} onChange={e => setDescription(e.target.value)} placeholder="Descrição (ex: Supermercado)" className="p-2 border rounded-md w-full" />
                <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="Valor Total (R$)" className="p-2 border rounded-md w-full" />
                <input type="text" value={tag} onChange={e => setTag(e.target.value)} placeholder="Tag (ex: Alimentação)" className="p-2 border rounded-md w-full" />
                <input type="date" value={date} onChange={e => setDate(e.target.value)} className="p-2 border rounded-md w-full" />
            </div>
            {type === 'monthly' && (
                <div>
                    <label htmlFor="installments" className="block text-sm font-medium text-gray-700">Número de Parcelas</label>
                    <input type="number" id="installments" value={installments} onChange={e => setInstallments(Math.max(1, parseInt(e.target.value) || 1))} min="1" className="p-2 border rounded-md w-full mt-1" />
                </div>
            )}
...
const TransactionList = ({ transactions, onEdit, onDelete }) => {
    ...
    if (transactions.length === 0) {
        return <p className="text-gray-500">Nenhuma transação para exibir.</p>;
    }

    return (
        <ul className="space-y-3">
            {transactions.map(t => (
                <li key={t.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg group">
                    <div>
                        <p className="font-semibold text-gray-800">{t.description}</p>
                        <p className="text-sm text-gray-500">{t.tag} - {t.date.toDate().toLocaleDateString('pt-BR')}</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <p className={`font-bold ${t.amount < 0 ? 'text-red-500' : 'text-green-500'}`}>{formatCurrency(t.amount)}</p>
                        <div className="flex opacity-0 group-hover:opacity-100 transition-opacity">
                             <button onClick={() => onEdit(t)} className="p-1 text-gray-500 hover:text-indigo-600"><PencilIcon className="w-4 h-4" /></button>
                             <button onClick={() => onDelete(t)} className="p-1 text-gray-500 hover:text-red-600"><TrashIcon className="w-4 h-4" /></button>
                        </div>
                    </div>
                </li>
            ))}
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white p-6 rounded-xl shadow-md text-center">
                    <h3 className="text-lg font-semibold text-gray-500">Orçamento Semanal</h3>
                    <div className="flex items-center justify-center gap-2 mt-2">
                        <span className="text-gray-500 font-semibold">R$</span>
                        <input type="number" value={newBudget} onChange={(e) => setNewBudget(e.target.value)} onBlur={handleBudgetUpdate} className="text-3xl font-bold text-indigo-600 bg-transparent border-b-2 border-indigo-200 focus:outline-none focus:border-indigo-500 text-center w-32" />
                    </div>
                </div>
                <div className={`bg-white p-6 rounded-xl shadow-md text-center ${remainingBudget < 0 ? 'text-red-600' : 'text-green-600'}`}>
                    <h3 className="text-lg font-semibold text-gray-500">Saldo Restante</h3>
                    <p className="text-3xl font-bold">{formatCurrency(remainingBudget)}</p>
                </div>
            </div>
            <div className="bg-white p-6 rounded-xl shadow-md"><TransactionForm householdId={householdId} type="weekly" /></div>
            <div className="bg-white p-6 rounded-xl shadow-md">
                <h3 className="font-bold text-xl mb-4">Gastos da Semana</h3>
        ...
        <div className="space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-md">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-xl">Gastos por Categoria</h3>
                ...
                ) : <p className="text-gray-500 text-center py-10">Sem dados de gastos para exibir no gráfico deste mês.</p>}
            </div>
            <div className="bg-white p-6 rounded-xl shadow-md">
                <div className="flex justify-between items-center mb-4"><h3 className="font-bold text-xl">Análise Financeira com IA</h3></div>
                ...
                {analysis && (<div className="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-lg whitespace-pre-wrap font-sans text-gray-700">{analysis}</div>)}
            </div>
            <div className="bg-white p-6 rounded-xl shadow-md"><TransactionForm householdId={householdId} type="monthly" /></div>
            <div className="bg-white p-6 rounded-xl shadow-md">
                <h3 className="font-bold text-xl mb-4">Todas as Transações do Mês</h3>
...
const Dashboard = ({ user, householdId, household, transactions }) => {
        ...
        <div className="bg-gray-100 min-h-screen">
            <header className="bg-white shadow-sm">
                <div className="max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex justify-between items-center">
                    <div className="flex items-center gap-3"><WalletIcon /><h1 className="text-2xl font-bold text-gray-900">OrçaCasal</h1></div>
                    <div className="flex items-center gap-4">
                        <div className="text-sm text-gray-600"><span>ID do Lar: </span><span className="font-mono bg-gray-200 p-1 rounded-md cursor-pointer" onClick={copyToClipboard} title="Clique para copiar">{householdId}</span></div>
                        <button onClick={handleSignOut} className="flex items-center gap-2 text-sm text-gray-600 hover:text-indigo-600 transition"><LogoutIcon />Sair</button>
                    </div>
                </div>
            </header>
            <main className="max-w-4xl mx-auto py-8 px-4">
                <div className="mb-6"><div className="border-b border-gray-200"><nav className="-mb-px flex space-x-8" aria-label="Tabs">
                    <button onClick={() => setActiveTab('weekly')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'weekly' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>Visão Semanal</button>
                    <button onClick={() => setActiveTab('monthly')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'monthly' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>Visão Mensal</button>
                </nav></div></div>
            ...
            <ConfirmationModal isOpen={!!deletingTransaction} onClose={handleCloseModals} onConfirm={handleDeleteConfirm} title="Confirmar Exclusão">
                ...
                {deletingTransaction?.isInstallment && (
                    <div className="mt-4 space-y-2">
                        <label className="flex items-center"><input type="radio" name="deleteScope" value="single" checked={deleteScope === 'single'} onChange={() => setDeleteScope('single')} className="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" /><span className="ml-2 text-sm text-gray-700">Excluir apenas esta parcela</span></label>
                        <label className="flex items-center"><input type="radio" name="deleteScope" value="all" checked={deleteScope === 'all'} onChange={() => setDeleteScope('all')} className="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" /><span className="ml-2 text-sm text-gray-700">Excluir esta e todas as parcelas futuras</span></label>
                    </div>
                )}
            </ConfirmationModal>
        </div>
...
export default function App() {
    ...
    // Efeito 1: Gerencia o estado de autenticação.
    useEffect(() => {
    ...
    // Efeito 2: Reage à mudança de usuário para buscar o ID do lar.
    useEffect(() => {
        if (!user) {
            setHouseholdId(null);
            return;
        ...
        const unsubscribeUser = onSnapshot(userDocRef, (docSnap) => {
            const userHouseholdId = docSnap.exists() ? docSnap.data().householdId : null;
    ...
    // Efeito 3: Reage à mudança do ID do lar para buscar os dados do lar e as transações.
    useEffect(() => {
        if (!householdId) {
            setHousehold(null);
            setTransactions([]);
    ...
    if (isLoading) {
        return <div className="min-h-screen flex items-center justify-center"><div className="animate-spin rounded-full h-32 w-32 border-t-2 border-b-2 border-indigo-500"></div></div>;
    }
    ...
    if (household) {
        return <Dashboard user={user} householdId={householdId} household={household} transactions={transactions} />;
    }
